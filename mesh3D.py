    ...
    for k in range(X.shape[2]):
        for j in range(X.shape[1]):
            for i in range(X.shape[0]):
                if raio(X[i][j][k],Y[i][j][k]) <= f(Z[i][j][k]):
                    pid = generate_pid(i,j,k)
                    
                    #test for front neighbor (x axis)
                    try:
                        if raio(X[i][j+1][k],Y[i][j+1][k]) <= f(Z[i][j+1][k]):
                    ...
                    try:
                        if raio(X[i][j-1][k],Y[i][j-1][k]) <= f(Z[i][j-1][k]) and (j-1)>=0:
                    ...
                    try:
                        if raio(X[i-1][j][k],Y[i-1][j][k]) <= f(Z[i-1][j][k])and (i-1)>=0:
                            left = generate_pid(i-1,j,k)
                        else: left = '-1'
                    except: left = '-1'
                    #test for right neighbor
                    try:
                        if raio(X[i+1][j][k],Y[i+1][j][k]) <= f(Z[i+1][j][k]):
                    ...
                    try:
                        if raio(X[i][j][k+1],Y[i][j][k+1]) <= f(Z[i][j][k+1]):
                            up = generate_pid(i,j,k+1)
                        else: up = '-1'
                    except: up = '-1'
                    #test for down neighbor (z axis)
                    try:
                        if raio(X[i][j][k-1],Y[i][j][k-1]) <= f(Z[i][j][k-1]) and (k-1)>=0:
                    ...
                    if Z[i][j][k] == 0:
                        ...
                        potential_value = 0
                    elif '-1' in [front,back,left,right,up,down]:
                        potential_value = 0
