    ...
    def compile_grad(self):
        ...
        for i,element in enumerate(mesh):
            ...
            mesh[i][idx_Jz] = -1*sigma*((list_temp_V[4] - list_temp_V[5]))/(h*int((CoefDiff[4]*CoefDiff[5])**.5))
                ...
                #mesh[i][idx_Jy] = -1*sigma*(mesh[right_neighbor][idx_V] - mesh[left_neighbor][idx_V])/(2*h)
                #mesh[i][idx_Jx] = -1*sigma*(mesh[front_neighbor][idx_V] - mesh[back_neighbor][idx_V])/(2*h)
                #mesh[i][idx_Jz] = -1*sigma*(mesh[up_neighbor][idx_V] - mesh[down_neighbor][idx_V])/(2*h)
    ...
    def plot2d(self,filename = 'plot2D', list_cuts1 = [0,1,2], list_cuts2 = [0,1,2]):
        ...
        for e,val in enumerate(x_1):
            ...
            try:
                ...
                c = ax[e].pcolormesh(y1, z1, v1, cmap=cmap, shading='gouraud',vmin=0, vmax=10,snap = True)
        ...
        for e,val in enumerate(z_1):
            ...
            try:
                ...
                c = ax[e].pcolormesh(x1, y1, v1, cmap=cmap, shading='gouraud',vmin=0, vmax=10,snap = True)
